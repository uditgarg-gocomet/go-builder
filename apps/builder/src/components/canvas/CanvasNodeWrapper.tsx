...
function CanvasNodeWrapperImpl({ nodeId, type, children }: CanvasNodeWrapperProps): React.ReactElement {
  ...
  return (
    ...
    >
      {/* Type badge — visible on hover or when selected. Pure CSS hover prevents
          mouseEnter/Leave loops with the negatively-positioned badge. */}
      <div
        {...attributes}
        {...listeners}
        onClick={e => e.stopPropagation()}
        className={[
          'absolute -top-2.5 left-2 z-50 hidden h-4 cursor-grab items-center rounded px-1.5 text-[10px] font-medium text-white shadow active:cursor-grabbing group-hover:flex',
