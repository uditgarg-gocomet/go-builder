// GET ${WORKFLOW_API}v1/{user_type}/workflow/document/verification

import type { DRDVEnvConfig } from '../env.js'
import { authHeaders, buildWorkflowUrl } from '../env.js'
import type { VerificationDetailsResponse } from '../../shared/types.js'
...
export async function fetchVerificationFromGocomet(
  ...
  const url = new URL(buildWorkflowUrl(config, 'workflow/document/verification'))
  url.searchParams.set('shipment_workflow_activity_id', request.swaId)
  url.searchParams.set('source_document_type', request.documentBucketId)
  ...
  return { ok: true, status: upstream.status, data: body as VerificationDetailsResponse }
}
